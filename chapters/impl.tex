...
The idea presented in the theory chapter \todo{reference and make sure I explained it correctly in theory}
is similar to memory segementation\todo{cont}

\subsection{Removing the Virtual Memory System}
Before implementing a new virtual memory system, the existing one must be remove from the kernel
modules.\\
The following parts of the code interact directly with the virtual memory system:
\begin{enumerate}
    \item \texttt{kernel/exec.c}
    \item \texttt{kernel/main.c}
    \item \texttt{kernel/proc.c}
    \item
\end{enumerate}

\subsection{Physical Memory Allocator} The virtual memory subsystem relies on the physical memory
...
\todo{changes to systemcalls? Esp. Fork, exec, sbrk}
