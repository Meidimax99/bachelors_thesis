...
\todo{add diff code listing for changes}
...
for a first proof of concept, it is helpful to not have to think about which registers to use and to optimize
...
After storing the state and running the exception handler, the state must again be restored with a number of
load instructions.
...
In between the store and load blocks, the machine\_default\_exception\_handler c-function is called. This is
